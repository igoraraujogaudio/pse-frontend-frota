...
import { AdicionarObraProgramacaoWizard } from '@/components/AdicionarObraProgramacaoWizard';
import { ObraProgramacaoEquipeService, ObraProgramacaoEquipe } from '@/services/obraProgramacaoEquipeService';
...
type SimpleActivity = { 
  id?: string;
  obraId?: string; // presente apenas para atividades do sistema obra_programacao_equipe
  fluxoDefinido?: boolean; // indica se o fluxo da etapa foi configurado
  programacaoOriginal?: ObraProgramacaoEquipe; // dados completos para abrir o modal de fluxo
  team: string; 
  osNumber: string; 
  value: number; 
...
const CONTRATO_GOIAS_ID = '03413132-fba2-459e-911f-478b9af69d21';
...
function ProgramacaoManutencoesContent() {
  ...
  const [selectedContrato] = useState<string>('Goiás');
  ...
  // Estado para wizard de adicionar obra à equipe
  const [addObraModal, setAddObraModal] = useState<{ equipe: EquipeComEncarregado; date: string } | null>(null);

  // Estado para modal de detalhes da obra
  const [selectedObraDetalhes, setSelectedObraDetalhes] = useState<ObraManutencao | null>(null);

  // Estado para modal de fluxo de etapa
  const [selectedFluxo, setSelectedFluxo] = useState<ObraProgramacaoEquipe | null>(null);

  const handleOpenObraDetalhes = async (obraId: string) => {
    try {
      const obra = await ObrasManutencaoService.getById(obraId);
  ...
  const getFilteredMapActivities = useCallback(() => {
    ...
    const equipesContrato = equipesFixas
      .filter(equipe => {
        return equipe.contratoId === CONTRATO_GOIAS_ID;
      })
  ...
  }, [mapActivities, mapFilterMonth, mapFilterYear, mapFilterDay, mapFilterStatus, mapFilterCritico, mapFilterTeam, equipesFixas, selectedContrato]);
  ...
  const loadActivities = useCallback(async () => {
    try {
      ...
      // Contrato fixo de Goiás
      const contratoId = CONTRATO_GOIAS_ID;
      ...
      // Mesclar obras da obra_programacao_equipe no calendário
      try {
        const [obrasProgEquipe, equipesAtuais] = await Promise.all([
          ObraProgramacaoEquipeService.getByWeek(startDate, endDate),
          contratoId ? EquipesComEncarregadoService.getEquipesComEncarregado(contratoId) : Promise.resolve(equipesFixas),
        ]);
        // Agrupar por data+equipe+obra para exibir uma única linha no calendário
        type GrupoKey = string;
        const grupos = new Map<GrupoKey, { op: typeof obrasProgEquipe[0]; etapas: string[] }>();
        obrasProgEquipe.forEach(op => {
          if (!op.obra || !op.id) return;
          const equipe = equipesAtuais.find(e => e.id === op.equipeId);
          const teamName = equipe ? (equipe.prefixo || equipe.nome) : '';
          if (!teamName) return;
          const key = `${op.data}|${op.equipeId}|${op.obraId}`;
          if (!grupos.has(key)) {
            grupos.set(key, { op, etapas: [] });
          }
          const etapa = op.etapa?.trim();
          if (etapa && !grupos.get(key)!.etapas.includes(etapa)) {
            grupos.get(key)!.etapas.push(etapa);
          }
        });

        grupos.forEach(({ op, etapas }) => {
          const equipe = equipesAtuais.find(e => e.id === op.equipeId);
          const teamName = equipe ? (equipe.prefixo || equipe.nome) : '';
          const etapaLabel = etapas.length > 0 ? `Etapa ${etapas.join(', ')}` : undefined;
          const simpleAct: SimpleActivity = {
            id: op.id!,
            obraId: op.obraId,
            fluxoDefinido: op.fluxoDefinido ?? false,
            programacaoOriginal: op,
            team: teamName,
            osNumber: op.obra!.numeroProjeto,
            value: op.obra!.valorProjetado ?? 0,
            status: ActivityStatus.PROG,
            location: op.obra!.enderecoObra ?? op.obra!.municipio ?? '',
            notes: etapaLabel ?? op.observacoes,
          };
          if (!activitiesByDate[op.data]) activitiesByDate[op.data] = [];
          activitiesByDate[op.data].push(simpleAct);
        });
      } catch (e) {
        console.warn('obra_programacao_equipe não disponível ainda:', e);
      }
  ...
  }, [currentYear, currentMonth, mapFilterYear, equipesFixas, dashboardStartYear, dashboardStartMonth, dashboardStartDay, dashboardEndYear, dashboardEndMonth, dashboardEndDay]);
  ...
  const loadEquipesFixas = useCallback(async () => {
    try {
      ...
      // Se a configuração ainda não carregou, não buscar equipes ainda
      // (evita race condition que traz todas as equipes antes da config carregar)
      if (contratoConfig === null) {
        console.log('⏳ Aguardando configuração do contrato carregar...');
        return;
      }
      ...
      if (contratoConfig?.equipesFixas && contratoConfig.equipesFixas.length > 0) {
        console.log(`🔍 Buscando equipes fixas para Goiás:`, contratoConfig.equipesFixas);
        
        // Buscar todas as equipes do contrato
        const todasEquipes = await EquipesComEncarregadoService.getEquipesComEncarregado(contratoId);
        ...
        const equipes = await EquipesComEncarregadoService.getEquipesComEncarregado(contratoId);
        setEquipesFixas(equipes);
        console.log(`✅ Usando ${equipes.length} equipes do contrato Goiás (sem equipes fixas configuradas)`);
      }
    } catch (err) {
      console.error('Erro ao carregar equipes:', err);
      setEquipesFixas([]);
    }
  }, [contratoConfig, selectedContrato]);
  ...
  // Carregar dados quando o componente monta ou o mês muda
  useEffect(() => {
    loadActivities();
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentYear, currentMonth, mapFilterYear]);
  ...
  const loadEncarregadosData = async () => {
    ...
    try {
      // Carregar equipes do contrato fixo de Goiás
      const equipes = await EquipesComEncarregadoService.getEquipesComEncarregado(CONTRATO_GOIAS_ID);
  ...
  const handleOpenAddObraModal = (equipe: EquipeComEncarregado, date: string) => {
    setAddObraModal({ equipe, date });
  };
  ...
  }, [dashboardStartMonth, dashboardStartYear, dashboardEndMonth, dashboardEndYear, activeTab, loadActivities]);
  ...
  return (
    <>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab}>
              <div className="flex items-center justify-center gap-8">
                ...
                <div className="flex items-center gap-1.5 flex-shrink-0">
                  <span className="px-3 py-1.5 text-sm font-semibold text-blue-700 bg-blue-50 border-2 border-blue-400 rounded-lg shadow-sm">
                    Goiás
                  </span>
                                  ...
                                  >
                                    ...
                                    <button
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        const dateStr = `${currentYear}-${(currentMonth + 1).toString().padStart(2, '0')}-${day.toString().padStart(2, '0')}`;
                                        handleOpenAddObraModal(equipe, dateStr);
                                      }}
                                      className="flex-shrink-0 w-3.5 h-3.5 rounded-full bg-blue-500 hover:bg-blue-700 text-white flex items-center justify-center leading-none"
                                  ...
                                  {atividadesDaEquipe.map((atividade, osIndex) => (
                                    <div
                                      ...
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        if (atividade.obraId) {
                                          handleOpenObraDetalhes(atividade.obraId);
                                        } else {
                                          setViewingActivity({ ...atividade, date: dateStr });
                                        }
                                      }}
                                      title={atividade.obraId && !atividade.fluxoDefinido ? 'Clique no SF para definir o fluxo' : undefined}
                                    >
                                      ...
                                      <div className="flex items-center justify-between py-0.5 min-w-max">
                                        <div className="flex items-center gap-1 flex-shrink-0">
                                          ...
                                          {atividade.obraId && !atividade.fluxoDefinido && (
                                            <button
                                              title="Sem fluxo — clique para definir"
                                              onClick={(e) => {
                                                e.stopPropagation();
                                                if (atividade.programacaoOriginal) setSelectedFluxo(atividade.programacaoOriginal);
                                              }}
                                              className="bg-red-500 text-white rounded font-bold hover:bg-red-700 transition-colors"
                                              style={{ fontSize: '7px', padding: '1px 3px', lineHeight: '1.2' }}
                                            >SF</button>
                                          )}
      ...
      {/* Wizard: Adicionar Obra à Equipe */}
      {addObraModal && (
        <AdicionarObraProgramacaoWizard
          equipe={addObraModal.equipe}
          date={addObraModal.date}
          onClose={() => setAddObraModal(null)}
          onSuccess={() => { setAddObraModal(null); loadActivities(); }}
        />
      )}

      {/* Modal de Fluxo da Etapa */}
      {selectedFluxo && (
        <FluxoEtapaModal
          programacao={selectedFluxo}
          isOpen={!!selectedFluxo}
          onClose={() => setSelectedFluxo(null)}
          onSaved={() => { setSelectedFluxo(null); loadActivities(); }}
        />
      )}

      {/* Modal de Detalhes da Obra (igual ao de obras-manutenção) */}
      <ObraDetalhesModal
        obra={selectedObraDetalhes}
        isOpen={!!selectedObraDetalhes}
        onClose={() => setSelectedObraDetalhes(null)}
        onStatusChange={(obraId, newStatus) => {
          setSelectedObraDetalhes(prev =>
            prev?.id === obraId ? { ...prev, status: newStatus as StatusObra } : prev
          );
          loadActivities();
        }}
      />
