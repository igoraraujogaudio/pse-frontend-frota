...
export default function DebugPermissoesPerfilPage() {
  const { user } = useAuth()
  ...
  useEffect(() => {
    if (user && userPermissions) {
      // Filtrar apenas permissões do perfil (não há tipo 'perfil_padrao' no sistema modular)
      // No sistema modular, permissões de perfil são as que não são individuais
  ...
  if (loading) {
    return (
      <div className="p-6">
        <h1 className="text-2xl font-bold mb-4">Debug Permissões do Perfil</h1>
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-6">Debug Permissões do Perfil - {user.nome}</h1>
      ...
      <div className="bg-green-50 p-4 rounded-lg mb-6">
        <h2 className="text-lg font-semibold mb-2">Resumo das Permissões do Perfil</h2>
        <div className="grid grid-cols-3 gap-4">
          <div className="text-center">
            <div className="text-2xl font-bold text-green-600">{permissoesPerfil.length}</div>
            <div className="text-sm text-gray-600">Total de Permissões</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-600">
              {permissoesPerfil.filter(p => p.concedido).length}
            </div>
        ...
        <div className="mt-4 text-center">
          <div className="text-lg font-semibold text-red-600">
            ⚠️ PROBLEMA: Deveria ter apenas 35 permissões, mas tem {permissoesPerfil.length}!
          </div>
        </div>
      </div>
          ...
          {permissoesPerfil.map((perm, index) => (
            <div key={index} className="bg-white p-3 rounded border">
              <div className="grid grid-cols-3 gap-2">
                ...
                <div>
                  <strong>Funcionalidade ID:</strong> {String(perm.funcionalidade_id)}
                </div>
              </div>
            </div>
          ))}
      ...
      <div className="bg-yellow-50 p-4 rounded-lg mb-6">
        <h2 className="text-lg font-semibold mb-2">Permissões do Perfil - Estoque</h2>
        <div className="space-y-2">
          {[
            ...
            const perm = permissoesPerfil.find(p => p.funcionalidade?.codigo === codigo)
            return (
      ...
      <div className="bg-gray-50 p-4 rounded-lg">
        <h2 className="text-lg font-semibold mb-2">Debug Completo - Permissões do Perfil</h2>
        <pre className="bg-white p-3 rounded border text-xs overflow-auto max-h-96">
          {JSON.stringify(permissoesPerfil, null, 2)}
        </pre>
      </div>
    </div>
  )
