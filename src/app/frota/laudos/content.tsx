...
// import { Vehicle, VehicleDocument } from '@/types'; // TODO: Use for type safety
// import { vehicleService } from '@/services/vehicleService'; // TODO: Implement vehicle service calls
...
// Mapeamento dos subtipos de laudo elétrico para exibição
...
// Função para formatar o tipo de documento com subtipo
...
// Função para calcular o status do documento - TODO: Use for status calculation
// function getStatus(expiresAt: string | null) {
//   if (!expiresAt) return { status: 'sem_data', label: 'Sem data de validade', color: 'bg-gray-100 text-gray-600' };
//   
...
export function LaudosContent() {
  const { user } = useAuth();
  const { hasPermission } = useModularPermissions();
  const searchParams = useSearchParams();
  
  // Verificações de permissão específicas
  const canManageLaudos = hasPermission(PERMISSION_CODES.LAUDOS.GERENCIAR_LAUDOS);
  const canViewAlerts = hasPermission(PERMISSION_CODES.LAUDOS.ALERTAS_VENCIMENTO);
  const canViewReports = hasPermission(PERMISSION_CODES.LAUDOS.RELATORIO_CONFORMIDADE);
  const canViewLaudos = hasPermission(PERMISSION_CODES.LAUDOS.VISUALIZAR_LAUDOS);
  
  // Parâmetros para destacar veículo específico (vindo de notificações)
  ...
  // Buscar laudos com regras aplicadas (API corrigida) - CACHE SUPER AGRESSIVO
  const {
    data: laudosData,
    isLoading: loading,
    error
    ...
    queryFn: async () => {
      try {
        ...
        if (!response.ok) {
          console.error('❌ Resposta não OK da API laudos:', response.status, response.statusText);
    ...
    retry: 1,
    staleTime: 5 * 60 * 1000, // 5 minutos - cache super agressivo
  ...
  useEffect(() => {
    if (error) {
      notify('Erro ao carregar laudos', 'error');
    }
  }, [error, notify]);

  // Processar dados dos laudos da API
  const laudos = laudosData?.laudos || [];
  
  // Filtrar laudos baseado no acesso do usuário
  const filteredLaudosByAccess = laudos.filter((laudo: { 
    ...
    try {
      ...
      console.error('Erro ao filtrar laudo:', err, laudo);
      return false;
    }
  });

  // Reset do filtro de contrato ao montar
  useEffect(() => {
    setContratoFilter([]);
  }, []);

  // Filtrar automaticamente pelo veículo destacado quando vindo de notificação
  useEffect(() => {
    if (highlightPlate && filteredLaudosByAccess.length > 0) {
      setSearch(highlightPlate);
      
      // Verificar se há documentos vencidos ou próximos do vencimento para este veículo
      ...
      // Aplicar filtro baseado no que existe para este veículo
      if (hasExpired) {
        setStatusFilter('vencido');
      } else if (hasExpiring) {
        setStatusFilter('proximo');
      } else {
        // Se não há documentos vencidos/próximos, mostrar todos para este veículo
        setStatusFilter('all');
      }
    }
  ...
  // Gerar lista de contratos únicos dos laudos filtrados
  ...
  // Filtrar laudos por busca, status e contrato
  const filteredLaudos = filteredLaudosByAccess.filter((laudo: Laudo) => {
    try {
      ...
      const matchesStatus = statusFilter === 'all'
        ? ['vencido', 'proximo', 'faltando', 'valido', 'sem_data'].includes(laudo.status)
        : statusFilter === 'em_dia' 
          ? (laudo.status === 'valido' || laudo.status === 'sem_data')
      ...
      console.error('Erro ao filtrar laudo na busca:', err, laudo);
      return false;
    }
  });
  ...
  // Calcular contadores usando a mesma lógica de filtro
  const getStatusCounts = () => {
    return filteredLaudosByAccess.reduce((acc: { vencidos: number; proximos: number; faltando: number; em_dia: number }, laudo: { 
      ...
      try {
        ...
        // Aplicar filtro de busca (mesmo que filteredLaudos)
        const matchesSearch = search === '' || 
          laudo.veiculo.placa?.toLowerCase().includes(search.toLowerCase()) ||
          laudo.veiculo.modelo?.toLowerCase().includes(search.toLowerCase()) ||
          laudo.veiculo?.tipo_veiculo?.toLowerCase().includes(search.toLowerCase()) ||
        ...
        // Só contar se passar em todos os filtros
        ...
        else if (laudo?.status === 'valido' || laudo?.status === 'sem_data') acc.em_dia++;
        return acc;
      } catch (err) {
        console.error('Erro ao contar status:', err, laudo);
        return acc;
      }
    ...
    return (
      <div className="flex items-center justify-center py-20">
        <div className="text-center">
          <div className="text-gray-500">Carregando laudos com regras aplicadas...</div>
          <div className="text-sm text-gray-400 mt-2">Processando documentos por veículo</div>
        </div>
      </div>
    );
  }
  ...
  // Função auxiliar para mapear status para valores simples
  ...
  // Função para exportar laudos para Excel
  const exportarExcel = async () => {
    try {
      setExportingExcel(true);
      
      // Preparar dados para Excel usando os laudos filtrados
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, 'Laudos');
      
      // Gerar nome do arquivo com data
      const dataAtual = new Date().toLocaleDateString('pt-BR').replace(/\//g, '-');
      const nomeArquivo = `Laudos_${dataAtual}.xlsx`;
  ...
  return (
    <div className="bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 py-2">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                <span className="text-sm text-gray-500">Sistema Modular de Laudos</span>
        ...
        {/* Mensagem informativa quando vindo de notificação */}
        {highlightPlate && (
            ...
            <div className="flex items-center">
              ...
              <div>
                <p className="text-sm font-medium text-blue-800">
                  Veículo destacado da notificação: <span className="font-bold">{highlightPlate}</span>
                </p>
                <p className="text-xs text-blue-600 mt-1">
                  {statusFilter === 'vencido' && 'Filtros aplicados automaticamente para mostrar documentos vencidos deste veículo.'}
                  {statusFilter === 'proximo' && 'Filtros aplicados automaticamente para mostrar documentos próximos do vencimento deste veículo.'}
                  {statusFilter === 'all' && 'Filtros aplicados automaticamente para mostrar todos os documentos deste veículo.'}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-4">
          <button
            ...
            className={`bg-white rounded-xl shadow-sm p-6 border transition-colors ${
              statusFilter === 'em_dia' ? 'border-green-200' : 'border-gray-200 hover:border-green-200'
            }`}
          >
          ...
          <button
            ...
            className={`bg-white rounded-xl shadow-sm p-6 border transition-colors ${
              statusFilter === 'vencido' ? 'border-red-200' : 'border-gray-200 hover:border-red-200'
            }`}
          >
            <div className="flex items-center justify-between">
              <div>
          ...
          <button
            ...
            className={`bg-white rounded-xl shadow-sm p-6 border transition-colors ${
              statusFilter === 'proximo' ? 'border-yellow-200' : 'border-gray-200 hover:border-yellow-200'
            }`}
          >
            <div className="flex items-center justify-between">
              <div>
          ...
          <button
            onClick={() => setStatusFilter(statusFilter === 'faltando' ? 'all' : 'faltando')}
            className={`bg-white rounded-xl shadow-sm p-6 border transition-colors ${
              statusFilter === 'faltando' ? 'border-gray-400' : 'border-gray-200 hover:border-gray-400'
            }`}
          >
            <div className="flex items-center justify-between">
              <div>
        ...
        {/* Barra de pesquisa e filtros juntos */}
        <div className="mb-3 flex flex-col gap-4">
          <div className="flex flex-col sm:flex-row gap-4 items-end">
            <div className="relative flex-1 min-w-0">
              ...
              <input
                type="text"
                placeholder="Buscar por placa, modelo, localização ou tipo de laudo..."
            ...
            <div className="flex flex-col w-full sm:w-64 relative">
              ...
              {uniqueContratos.length > 1 ? (
                ...
                <Select 
                  value={contratoFilter.length > 0 ? contratoFilter[0] : "all"} 
                  onValueChange={(value) => {
                    setContratoFilter(value === "all" ? [] : [value]);
                  }}
                >
                  <SelectTrigger className="w-full">
                    <SelectValue placeholder="Todos os Contratos">
                      {contratoFilter.length === 0 ? 'Todos os Contratos' : contratoFilter[0]}
                    </SelectValue>
                  </SelectTrigger>
                  <SelectContent>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {paginatedLaudos.map((laudo: { 
                  ...
                  // Verificar se este veículo deve ser destacado (vindo de notificação)
                  ...
                  return (
                    ...
                    <td className="px-4 py-1.5 whitespace-nowrap">
                      ...
                      {/* Debug: sempre mostrar subtipo para laudos elétricos */}
                      {laudo.tipo_documento === 'eletrico' && (
                        <div className="text-xs text-gray-500 mt-1">
                          <span className={`px-2 py-1 rounded-full text-xs ${
                            laudo.subtipo_documento === 'lanca_isolada' ? 'bg-blue-100 text-blue-800' :
                            laudo.subtipo_documento === 'liner' ? 'bg-green-100 text-green-800' :
                            'bg-gray-100 text-gray-800'
                          }`}>
                            {laudo.subtipo_documento ? 
                              (subtipoLabels[laudo.subtipo_documento] || laudo.subtipo_documento) : 
                              'Geral (sem subtipo)'
                            }
                          </span>
                        </div>
                    ...
                    <td className="px-4 py-1.5 whitespace-nowrap">
                      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${laudo.statusColor}`}>
                        {laudo.status === 'vencido' && <ExclamationTriangleIcon className="h-4 w-4 mr-1" />}
                        {laudo.status === 'faltando' && <ExclamationTriangleIcon className="h-4 w-4 mr-1" />}
                    ...
                    <td className="px-4 py-1.5 whitespace-nowrap">
                      <div className="flex gap-2 justify-center">
                        {/* Botão Atualizar - apenas para quem pode gerenciar laudos */}
                        {laudo.status !== 'faltando' && canManageLaudos && (
                          <button
                            onClick={() => {
                              setShowUploadModal(laudo.id || null);
                        ...
                        {/* Botão Ver PDF - para quem pode visualizar laudos */}
                        {laudo.url_arquivo && canViewLaudos && (
                          <a
                            href={laudo.url_arquivo}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="inline-flex items-center px-2 py-1 border border-transparent text-xs font-medium rounded text-gray-700 bg-gray-100 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                            title="Ver PDF"
                          >
                        ...
                        {/* Botão Adicionar - apenas para quem pode gerenciar laudos */}
                        {laudo.status === 'faltando' && canManageLaudos && (
                          <button
                            onClick={() => {
                              setShowUploadModal(laudo.id || null);
